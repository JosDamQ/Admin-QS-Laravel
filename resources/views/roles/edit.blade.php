                ...
                <div class="bg-white dark:bg-gray-800 overflow-hidden shadow-sm sm:rounded-lg">
                    <div class="p-6 text-gray-900 dark:text-gray-100">
                        <div class="w-full h-full"> <!-- Elimina la clase max-w-xs -->            
                            <form method="POST" action="{{ route('roles.update', $role) }}" class="bg-slate-700 shadow-md rounded px-8 pt-6 pb-8 mb-4 w-full h-full">
                                @csrf @method('PUT')
                                <div class="mb-4">
                                    <label for="name" class="block text-white-700 text-sm font-bold mb-2">
                                        Name
                                    </label>
                                    <input class="shadow appearance-none border rounded w-full py-2 px-3 text-gray-700 leading-tight focus:outline-none focus:shadow-outline" id="name" name="name" type="text" placeholder="Name" value="{{ $role->name }}">
                                    ...
                                    <div class="grid grid-cols-2 gap-4">
                                        @foreach($permissions as $permission)
                                            <div class="flex items-center">
                                                <input id="permission{{ $permission->id }}" name="permissions[]" type="checkbox" class="form-checkbox h-5 w-5 text-blue-600" value="{{ $permission->id }}" {{ $role->permissions->contains($permission->id) ? 'checked' : '' }}>
